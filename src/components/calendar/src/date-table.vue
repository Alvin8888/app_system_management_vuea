...
export default {
  ...
  methods: {
    ...
    pickDay({ text, type }) {
      // 判断是否到当前月份，是当前月则不能到下月
      const nowMouth = this.date.getMonth() + 1
      const nextMouth = new Date().getMonth() + 1
      const calendarDate = window.localStorage.getItem('calendarDate')
      if (type == 'next' && nextMouth == nowMouth) return false
      if (type == 'prev' && nowMouth == calendarDate) return false
      const date = this.getFormateDate(text, type)
      this.$emit('pick', date)
    },
